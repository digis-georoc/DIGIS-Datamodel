...
-----------------------------------------------------------------------------------------------------------------------------
-- Alterations for GEOROC ---------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------

-- Paket1: People & Institutions
alter table ODM2.people
	alter column personfirstname drop not null,
	drop column personmiddlename;

alter table ODM2.organizations
	add organizationdepartment varchar(255) null,
	add organizationaddress1 varchar(255) null,
	add organizationaddress2 varchar(255) null,
	add organizationcitystatezip varchar(255) null,
	add organizationdecountry varchar(255) null,
	alter column organizationtypecv drop not null,
	alter column organizationcode drop not null,
	alter column organizationname drop not null;

alter table ODM2.affiliations
	drop column isprimaryorganizationcontact,
	drop column primaryaddress,
	drop column personlink,
	alter column affiliationstartdate drop not null,
	alter column affiliationenddate drop not null,
	alter column primaryphone drop not null,
	alter column primaryemail drop not null;

-- Paket2: Citations & External Identifiers
alter table odm2.citations
	add journal varchar(255) null,
	add volume varchar(255) null,
	add issue varchar(255) null,
	add firstPage varchar(255) null,
...
alter table odm2.specimens
	add column SpecimenTexture varchar(255) null;

create table odm2.SpecimenAges (
	SamplingFeatureID int not null,
	SpecimenAgeMin float8 null,
	SpecimenAgeMax float8 null,
	SpecimenGeolAge varchar(255) null,
	SpecimenGeolAgePrefix varchar(255) null,
	EruptionDay int null,
	EruptionMonth int null,
	EruptionYear int null
);


alter table odm2.samplingfeatures
...
alter table odm2.units
	add column UnitGEOROC varchar not null;

alter table odm2.datasets
	alter column datasetuuid drop not null,
...
alter table odm2.results
    alter column valuecount drop not null;
...
alter table odm2.referencematerialvalues
	drop constraint fk_referencematerialvalues_citations,
	alter column unitsid drop not null;

create table odm2.FractCorrect(
	FractCorrectID int primary key,
	FractCorrectValue float8 not null,
	FractCorrectVariable varchar(255) not null
);

create table odm2.ActionsFractCorrect(
	BridgeID serial primary key,
